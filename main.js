...
function initBgCanvas() {
  ...
  function setup() {
    ...
    orbs = [
      { x: W * 0.1, y: H * 0.2, vx: 0.15, vy: 0.1, r: W * 0.22, col: '0,229,192', a: 0.055 },
      { x: W * 0.85, y: H * 0.15, vx: -0.12, vy: 0.14, r: W * 0.20, col: '0,180,255', a: 0.04 },
      { x: W * 0.5, y: H * 0.8, vx: 0.1, vy: -0.12, r: W * 0.18, col: '0,229,192', a: 0.035 },
      { x: W * 0.75, y: H * 0.6, vx: -0.1, vy: 0.1, r: W * 0.16, col: '0,100,200', a: 0.04 },
    ].map(o => ({ ...o, phase: Math.random() * Math.PI * 2, sp: 0.0003 + Math.random() * 0.0002 }));

    // Puntos de grid tenue
    dots = [];
    const spacing = 52;
    for (let x = 0; x < W; x += spacing) {
      for (let y = 0; y < H; y += spacing) {
        dots.push({ x, y, baseA: Math.random() * 0.18 + 0.04 });
      }
    }
  }
  ...
  function draw(t) {
    ...
    orbs.forEach(o => {
      ...
      const pulse = 1 + 0.05 * Math.sin(t * o.sp + o.phase);
      const g = ctx.createRadialGradient(o.x, o.y, 0, o.x, o.y, o.r * pulse);
      g.addColorStop(0, `rgba(${o.col},${o.a})`);
      g.addColorStop(0.5, `rgba(${o.col},${o.a * 0.3})`);
      g.addColorStop(1, `rgba(${o.col},0)`);
      ctx.fillStyle = g;
      ctx.fillRect(0, 0, W, H);
    });

    // Grid de puntos tenue
    dots.forEach(d => {
      const dist = Math.hypot(mx - d.x, my - d.y);
      const glow = dist < 180 ? (1 - dist / 180) * 0.55 : 0;
      const a = d.baseA + glow;
      ctx.beginPath();
    ...
    gm.addColorStop(0, 'rgba(0,229,192,0.07)');
    gm.addColorStop(1, 'rgba(0,229,192,0)');
    ctx.fillStyle = gm;
    ctx.fillRect(0, 0, W, H);
...
function viewFile(url, ext, fileName) {
  ...
  const isPDF = ext === 'PDF'
  const isImage = ['PNG', 'JPG', 'JPEG', 'GIF', 'WEBP'].includes(ext)
  const isCode = ['SQL', 'TXT', 'JS', 'HTML', 'CSS', 'JSON', 'XML', 'PY'].includes(ext)
  ...
  if (isPDF) {
    ...
    content = `<div class="file-viewer-content code-viewer" id="codeViewerContent"><div class="code-loading">Cargando...</div></div>`
  } else {
      ...
      fetch(url)
        ...
        .then(text => {
          ...
          function highlight(code) {
            const keywords = ['SELECT', 'FROM', 'WHERE', 'INSERT', 'INTO', 'UPDATE', 'DELETE', 'CREATE', 'DROP', 'ALTER', 'TABLE', 'DATABASE', 'INDEX', 'VIEW', 'SET', 'ON', 'OFF', 'GO', 'USE', 'WITH', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER', 'GROUP BY', 'ORDER BY', 'HAVING', 'DISTINCT', 'AS', 'AND', 'OR', 'NOT', 'NULL', 'PRIMARY KEY', 'FOREIGN KEY', 'REFERENCES', 'DEFAULT', 'CONSTRAINT', 'VALUES', 'BEGIN', 'END', 'IF', 'ELSE', 'RETURN', 'EXEC', 'PROCEDURE', 'FUNCTION', 'TRIGGER', 'DECLARE', 'NOCOUNT']
            ...
            // GO statements
            escaped = escaped.replace(/\bGO\b/g, '<span class="sql-go">GO</span>')
            // Keywords
            keywords.forEach(kw => {
              const re = new RegExp(`\\b${kw}\\b`, 'gi')
              escaped = escaped.replace(re, m => `<span class="sql-keyword">${m.toUpperCase()}</span>`)
            })
            // Numbers
            escaped = escaped.replace(/\b(\d+)\b/g, '<span class="sql-number">$1</span>')
            // Table names in brackets
            escaped = escaped.replace(/\[([^\]]+)\]/g, '[<span class="sql-table">$1</span>]')
            return escaped
          }
  ...
  <div class="code-viewer-dots">
    ...
    `
        ...
        .catch(() => {
          const el = document.getElementById('codeViewerContent')
          if (el) el.innerHTML = `<p style="color:#ff6b6b;padding:1rem">No se pudo cargar el archivo</p>`
        })
    }, 100)
  }
